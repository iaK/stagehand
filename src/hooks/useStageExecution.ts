  ...
  const finalizeExecution = useCallback(
    async (
      ...
      if (wasKilled || (exitCode !== 0 && exitCode !== null)) {
        ...
        if (!wasKilled) {
          sendNotification("Stage failed", `${stage.name} encountered an error`, { projectId: activeProject.id, taskId: task.id });
        }
      } else {
        ...
        sendNotification("Stage complete", `${stage.name} needs your review`, { projectId: activeProject.id, taskId: task.id });
      }
...
Keep it under 72 characters for the first line. Add a blank line and body if needed.`;
      ...
      sendNotification("Ready to commit", `${stage.name} has changes to commit`, { projectId: activeProject.id, taskId: task.id });
    },
    [activeProject],
  );
  ...
  const createPullRequest = useCallback(
    async (task: Task, decision?: string) => {
      ...
      if (prUrl) {
        ...
        sendNotification("PR created", title, { projectId: activeProject.id, taskId: task.id });
      }
    },
