    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          {fixes.length} comment{fixes.length !== 1 ? "s" : ""} in {groups.length} review{groups.length !== 1 ? "s" : ""}
        ...
        <div className="flex-1 min-w-0 flex items-center gap-2 flex-wrap">
          ...
          {hasComments && group.allResolved && (
            <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400">
              ...
              <span className="text-[10px] font-medium uppercase">All resolved</span>
            </span>
          )}
        </div>
...
const CommentCard = memo(function CommentCard({
  ...
  const streamOutput = useProcessStore(
    (s) => (isFixing || isConsidering) ? (s.stages[sk]?.streamOutput ?? EMPTY_LINES) : EMPTY_LINES,
  );
  ...
  return (
    <div
      className="transition-colors rounded-md border border-border bg-background overflow-hidden"
    >
      ...
      >
        ...
        <Badge variant={fixInfo.badge} className="text-[10px] uppercase font-bold flex-shrink-0">
          {fixInfo.label}
          {fix.fix_commit_hash && ` (${fix.fix_commit_hash})`}
        </Badge>

        {fix.comment_type !== "review" && (
          <Badge variant="secondary" className="text-[10px] uppercase font-medium flex-shrink-0">
            {fix.comment_type}
          </Badge>
        )}
        ...
        {replies.length > 0 && (
          <span className="flex items-center gap-1 text-muted-foreground flex-shrink-0">
            ...
            <span className="text-[10px]">{replies.length}</span>
          </span>
        )}
      ...
      {expanded && (
        <div className="px-3 pb-3 pt-1">
          ...
          {consideration && !isConsidering && (
            <div className="mt-3 rounded-md border border-amber-200 dark:border-amber-500/20 bg-amber-50/50 dark:bg-amber-500/5 p-3">
              ...
              <div className="text-sm">
                <TextOutput content={consideration} />
              </div>
            </div>
          )}

          {/* Stream output while fixing or considering */}
          {(isFixing || isConsidering) && streamOutput && streamOutput.length > 0 && (
  ...
  return (
    <div className="flex gap-2">
      {reply.user.avatar_url ? (
        ...
        <div className="w-5 h-5 rounded-full bg-zinc-300 dark:bg-zinc-600 flex-shrink-0 mt-0.5 flex items-center justify-center text-[9px] font-medium text-zinc-600 dark:text-zinc-400">
